import { MaybeUndefined, ISelectionContainer, Disposable, MaybeArray, AnyObject, MaybePromise, EventHandler, CompositeDisposable, Guid, Mode, DisposableUrl, IComparable, IEquatable } from '../common' 
import { EtchComponent, StructColumns, StructEditorSettings, StructItem, StructItemCancelableEventArgs, StructItemCellEventArgs, StructItemEventArgs, StructItemKind, VirtualItem, StructEditor, StructItemCellDefault, StructColumn, StructItemCell, FileUploaderDialog, FileUploaderDialogBaseOptions, UploadFunction, DialogResult } from '../common-components' 
import { KBObject, KBModel, KBObjectInfo, KBEnvironment, EntityKey, KBObjectPart, KBObjectDescriptor, RemotePropertiesObject, IGXService, IKBObject, EntityVersionInfo, KnowledgeBase, KBCategory } from '../architecture-common' 
import { PropertyValueChangedEventArgs, IPropertyDescriptor, StringConverter, TypeDescriptorContext, UITypeEditor, UITypeEditorEditOptions, UITypeEditorEditStyle } from '../common-properties' 
import { TeamData, KBInfoData } from '../common-comm-layer' 
...
export interface IPropertyInspector {
    ...
    canResetSelectedProperty(): boolean;
    getSelectedPropertyValue(): MaybePromise<string | undefined>;
...
export interface IReportContentHandler {
    processEventHandlers(element: HTMLElement): MaybePromise<void>;
}
...
export declare type ReportToolWindowHeaderData = HeaderData;
export declare class ReportToolWindow extends AbstractToolWindow {
    private _key;
    private _name;
    private _itemsCallback;
    private _navigationReport;
    private _navigationReportContent?;
    private _reportToolContentHandler;
    private _onLoadedQueue;
    constructor(name: string, contentHandler: IReportContentHandler, iconName?: string);
    get name(): string;
    setHeaderData(headerData: ReportToolWindowHeaderData): Promise<void>;
    getHeaderData(): ReportToolWindowHeaderData;
    addItem(item: ReportItem, parentId: string): Promise<void>;
    clear(): Promise<void>;
    setHtmlContent(html: string, extraCss?: string[]): void;
    onReportLoaded(callback: Function): Promise<void>;
    private _itemSelected;
    private _addItem;
    private _setHeaderData;
...
export interface IModelTreeNode extends ICommandTarget {
    ...
    getDragData(): MaybePromise<string | undefined>;
...
export declare type ModelTreeDragDataItem = {
    metadata: string;
};
export declare type ModelTreeDraggableObject = {
    objectGuid: string;
};
export declare type ModelTreeDragData = ModelTreeDragDataItem[];
...
export declare abstract class AbstractNode implements IModelTreeNode {
    ...
    getDragData(): Promise<string | undefined>;
...
export declare class KBObjectNode extends AbstractNode {
    ...
    getDragData(): Promise<string | undefined>;
    getContextMenuPath(): string[] | undefined;
    reloadNodes(): Promise<any[]>;
    /**
     *
     * @override
...
export declare namespace CommonCommands {
    ...
    const OPEN_KB_FROM_SERVER: CommandDescriptor;
    const CREATE_OBJECT: {
        id: string;
        label: string;
        keybinding: string;
...
export declare type RemoteQuestionAnswer = {
    action: 'answer' | 'none';
    data?: any;
};
export interface IRemoteQuestionResolver {
    resolve(id: Guid, data: any): MaybePromise<RemoteQuestionAnswer>;
}
export declare const IRemoteQuestionResolverDescriptor: unique symbol;
export interface IRemoteQuestionResolverDescriptor {
    type: Guid;
    factory: () => IRemoteQuestionResolver;
}
...
export declare type ReloadNodeInfo = {
    skipReload: boolean;
...
export interface IModelTreeNodesProvider {
    ...
    getReloadNodeInfo(type: Guid, parentKey?: EntityKey): MaybePromise<ReloadNodeInfo | undefined>;
}
...
export declare class GXDocumentEditor extends EtchComponent implements IGXDocumentEditor {
    ...
    save(): Promise<void>;
    beforeSave(callback: () => void): Disposable;
    afterSave(callback: (success: boolean) => void): Disposable;
...
export interface IAutomatablePartEditor {
    setContent(content: string): MaybePromise<void>;
}
export declare namespace IAutomatablePartEditor {
    function is(arg: any): arg is IAutomatablePartEditor;
}
...
export declare class CollectionStructEditor extends StructEditorPlus {
    ...
    protected processDeletedMode(entity: KBObject): Promise<boolean>;
...
export declare type StructPartEditorPath = {
    path: number[];
    column?: string;
};
export declare abstract class StructPartEditor extends StructEditorPlus implements IGXDocumentPartEditor {
    ...
    activate(): MaybePromise<void>;
    deactivate(): MaybePromise<void>;
    get currentPosition(): StructPartEditorPath | undefined;
    setCurrentPosition(position: any): Promise<void>;
}

export declare class KBObjectCollectionEditor extends CollectionStructEditor implements ICommandTarget {
    private _typeId;
    protected internalItemKind: StructItemKind;
    protected itemKindId: string;
    protected itemsMap: Map<string, StructItem>;
    protected filter: string | undefined;
    protected module: KBObjectInfo | undefined;
    ...
    protected loadObjects(): Promise<void>;
...
export interface IComparerService {
    ...
    registerComparablePart(partGuid: Guid): MaybePromise<void>;
}
...
export declare type TeamInfo = TeamData;
...
export declare type CurrentUserChangedEventHandler = (currentUser?: UserInfo) => void;
...
export interface IEnvironmentService extends IGXService {
    ...
    currentUserChanged(callback: CurrentUserChangedEventHandler): Disposable;
...
export declare type KBEventHandler = (kb: MaybeUndefined<KnowledgeBase>) => MaybePromise<void>;
export declare type KBModelEventArgs = {
    previousModel: KBModel;
    model: KBModel;
};
export declare type KBModelEventHandler = (args: KBModelEventArgs) => MaybePromise<void>;
...
export interface IKBService extends IGXService {
    ...
    open(data: KBInfoData): MaybePromise<KnowledgeBase | undefined>;
    closeCurrentKB(): MaybePromise<boolean>;
    kbCreated(callback: KBEventHandler): Disposable;
    /**
     * Called after a Knowledge Base is created or opened.
     */
    currentKBChanged(callback: KBEventHandler): Disposable;
    /**
     * Called after a Model is created or opened.
     */
    currentModelChanged(callback: KBModelEventHandler): Disposable;
}
